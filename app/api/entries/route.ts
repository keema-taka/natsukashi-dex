...
const DISCORD_BOT_TOKEN   = process.env.DISCORD_BOT_TOKEN;   // Bot token "Bot xxxxxx" 形式で使う
...
export async function GET() {
  try {
    ...
    // 自アプリ由来のメッセージだけに絞る（footer.text を目印に）
    const mine = (json as any[])
      .filter(m => (m.embeds?.[0]?.footer?.text ?? "") === "natsukashi-dex")
      .map(mapDiscordMessageToEntry);
